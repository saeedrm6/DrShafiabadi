...
class SFWD_Update_Checker extends PluginUpdateChecker {
	...
	function key_warning() {
		...
		if ( !$this->check_license_key( $license_key ) )
			$msg = "<p><strong>" . sprintf( __('%s تقریبا آماده است.'), $this->plugin_name ) . "</strong> " 
				. sprintf(__('شما باید <a href="%s">یک لایسنس معتبر </a>را برای شروع عملکرد افزونه داشته باشید'), "admin.php?page={$this->options_page}" )
				. __( ' به لایسنس معتبر نیاز دارید ؟', 'all_in_one_seo_pack' ) 
				. ' <a href="http://www.prostyle.ir/support/" target="_blank">' . __( 'تیکتی ارسال کنید', 'all_in_one_seo_pack' ) . '</a>';
		else
			if ( $this->has_update() )
